...
/**
 * Per-stock detail page. Reads `ticker` from the route params and composes
 * `StockHeader` + `PriceTicker` + `StockChart` + `RawDataTable` (graph card
 * above raw-data card, per the wireframe), passing `ticker` down to each.
...
export function StockDetailPage(_props: StockDetailPageProps): JSX.Element {
  ...
  return (
    <div className={`stock-detail-page${isError ? ' is-disconnected' : ''}`}>
      ...
      <RawDataTable ticker={tickerValue} />
    </div>
  )
}
