...
describe('query key builders', () => {
  ...
  it('builds ticker-scoped keys for stock history, nested under the stock key', () => {
    expect(historyKey('AZN.L')).toEqual(['stock', 'AZN.L', 'history'])
...
describe('useStocksQuery', () => {
  ...
  it('is registered under the stocks query key with a 20s refetchInterval', async () => {
    ...
    await waitFor(() => {
      ...
      expect(query?.options.refetchInterval).toBe(POLL_INTERVAL_MS)
    })
  })
})
  ...
  beforeEach(() => {
    ...
    mockedApiGet.mockImplementation(async (path: unknown) => {
      ...
      for (const [ticker, history] of Object.entries(mockHistoryByTicker)) {
        if (p === `/api/stocks/${ticker}/history`) return history
      }
